...
export type RequestDecorator = {
	...
	context: Context;
	tracer: Tracer;
	inject<Carrier>(carrier: Carrier, setter?: typeof defaultTextMapSetter): void;
	extract<Carrier>(
		carrier: Carrier,
...
async function openTelemetryPluginImplementation(
	...
	function openTelemetryRequestDecorator(
		...
		return {
			...
			inject<Carrier>(carrier: Carrier, setter = defaultTextMapSetter) {
				return propagation.inject(getContext(request), carrier, setter);
			},
			extract<Carrier>(carrier: Carrier, getter = defaultTextMapGetter) {
	...
	async function onRequest(request: FastifyRequest) {
		if (shouldIgnoreRoute(request.url, request.method)) {
			return;
		}
	...
	if (wrapRoutes) {
		// still better to have async function than callback
		fastify.addHook("onRoute", async (routeOpts) => {
